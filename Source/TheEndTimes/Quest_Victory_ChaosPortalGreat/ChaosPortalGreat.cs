...
{
    public class ChaosPortalGreat : Building, IAttackTarget, IAttackTargetSearcher, ILoadReferenceable
    {
        ...
        public const float MaxSpawnedPawnsPoints = 3000f;
        public const float InitialPawnsPoints = 2500f;
        public const int PawnSpawnCountMin = 2;
        public const int PawnSpawnCountMax = 6;
        private static readonly FloatRange PawnSpawnIntervalDays = new FloatRange(0.001f, .01f); 
            ...
            get
            {
                ...
                if (base.Spawned)
                {
                    if (foundPawn == null)
                    {
                        ...
                        {
                            List<Thing> list = r.ListerThings.ThingsOfDef(ThingDef.Named("RH_TET_ChaosPortal_Great"));
                            for (int i = 0; i < list.Count; i++)
                            {
                                if (list[i] != this)
                                {
                                    ...
                                    {
                                        foundPawn = ((ChaosPortalGreat)list[i]).spawnedPawns.Find(hasDefendPortalLord);
                                        if (foundPawn != null)
                                        {
                                            return true;
                                        }
        ...
        {
            ChaosPortalGreat.spawnablePawnKinds.Clear();
            ChaosPortalGreat.spawnablePawnKinds.Add(PawnKindDef.Named("RH_TET_ChaosSpawn"));
            ChaosPortalGreat.spawnablePawnKinds.Add(PawnKindDef.Named("RH_TET_ChaosSpawnAncient"));
            ChaosPortalGreat.spawnablePawnKinds.Add(PawnKindDef.Named("RH_TET_Jabberslythe"));
            ChaosPortalGreat.spawnablePawnKinds.Add(PawnKindDef.Named("RH_TET_Razorgor"));
            ChaosPortalGreat.spawnablePawnKinds.Add(PawnKindDef.Named("RH_TET_Tuskgor"));
        ...
        public override void Tick()
        {
            ...
            if (base.Spawned)
            {
                ...
                {
                    int rando = Rand.Range(PawnSpawnCountMin,PawnSpawnCountMax);
        ...
        public override void DeSpawn(DestroyMode mode = DestroyMode.Vanish)
        {
            ...
            {
                lords[i].ReceiveMemo(ChaosPortalGreat.MemoDeSpawned);
            }
        ...
        public override void PostApplyDamage(DamageInfo dinfo, float totalDamageDealt)
        {
            if (dinfo.Def.ExternalViolenceFor(this) && dinfo.Instigator != null && dinfo.Instigator.Faction != null)
            {
                ...
                if (lord != null)
                {
                    lord.ReceiveMemo(ChaosPortalGreat.MemoAttackedByEnemy);
                }
            }
            if (dinfo.Def == DamageDefOf.Flame && (float)this.HitPoints < (float)base.MaxHitPoints * 0.3f)
            {
                ...
                if (lord2 != null)
                {
                    lord2.ReceiveMemo(ChaosPortalGreat.MemoBurnedBadly);
                }
            }
        ...
        public override void Kill(DamageInfo? dinfo = null, Hediff exactCulprit = null)
        {
            if (base.Spawned && (!dinfo.HasValue || dinfo.Value.Category != DamageInfo.SourceCategory.Collapse))
            {
                ...
                for (int i = 0; i < lords.Count; i++)
                {
                    lords[i].ReceiveMemo(ChaosPortalGreat.MemoDestroyedNonRoofCollapse);
                }
            }
        ...
        {
            ChaosPortalGreat.ResetStaticData();
            this.active = true;
            this.SpawnInitialPawns();
            this.CalculateNextPawnSpawnTick();
        }
        ...
        private void CalculateNextPawnSpawnTick()
        {
            ...
            this.nextPawnSpawnTick = Find.TickManager.TicksGame + (int)(ChaosPortalGreat.PawnSpawnIntervalDays.RandomInRange * 60000f / (num * Find.Storyteller.difficulty.enemyReproductionRateFactor));
        }
        ...
        private bool TrySpawnPawn(out Pawn pawn)
        {
            ...
            IEnumerable<PawnKindDef> source = from x in ChaosPortalGreat.spawnablePawnKinds
