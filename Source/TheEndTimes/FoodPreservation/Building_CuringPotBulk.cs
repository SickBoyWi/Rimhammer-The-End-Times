...
namespace TheEndTimes
{
    ...
    public class Building_CuringPotBulk : Building
    {
        ...
        private Material BarFilledMat
        {
            get
            {
                ...
                {
                    this.barFilledCachedMat = SolidColorMaterials.SimpleSolidColorMaterial(Color.Lerp(Building_CuringPotBulk.BarZeroProgressColor, Building_CuringPotBulk.BarFermentedColor, this.Progress), false);
                }
        ...
        public void AddStuff(int count)
        {
            ...
            if (this.Ready)
            {
                Log.Warning("Tried to add meat to a bulk curing pot that is already fully cured.");
                return;
            }
        ...
        public override string GetInspectString()
        {
            ...
            if (!this.Empty && !comp.Ruined)
            {
                if (this.Ready)
                {
                    stringBuilder.AppendLine("RH_TET_ContainsCuredFood".Translate(new object[]
                    {
                        this.stuffCount,
                        MaxCapacity
                    }));
                ...
                {
                    stringBuilder.AppendLine("RH_TET_ContainsCuring".Translate(new object[]
                    {
                        this.stuffCount,
                        MaxCapacity
                    }));
            ...
            if (!this.Empty)
            {
                if (this.Ready)
                {
                    stringBuilder.AppendLine("RH_TET_Cured".Translate());
                }
                else
                {
                    stringBuilder.AppendLine("RH_TET_CuringProgress".Translate(new object[]
                    {
                    ...
                    {
                        stringBuilder.AppendLine("RH_TET_CuringPotOutOfIdealTemperature".Translate(new object[]
                        {
            ...
            {
                "RH_TET_IdealCuringTemperature".Translate(),
                ": ",
        ...
        public Thing TakeOutStuff()
        {
            if (!this.Ready)
            {
                Log.Warning("Tried to get cured meat, but it's not yet cured.");
                return null;
            }
            Thing thing = ThingMaker.MakeThing(ThingDef.Named("RH_TET_CuredMeat"), null);
        ...
        protected override void DrawAt(Vector3 drawLoc, bool flip = false)
        {
            ...
            {
                center = vector3,
                size = Building_CuringPotBulk.BarSize,
                fillPercent = (float)this.stuffCount / 25f,
                filledMat = this.BarFilledMat,
                unfilledMat = Building_CuringPotBulk.BarUnfilledMat,
                margin = 0.1f,
                rotation = Rot4.North
            });
        }
