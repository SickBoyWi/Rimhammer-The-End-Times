...
namespace TheEndTimes
{
    ...
    public class Building_PicklingCrock : Building
    {
        ...
        private Material BarFilledMat
        {
            get
            {
                ...
                {
                    this.barFilledCachedMat = SolidColorMaterials.SimpleSolidColorMaterial(Color.Lerp(Building_PicklingCrock.BarZeroProgressColor, Building_PicklingCrock.BarFermentedColor, this.Progress), false);
                }
        ...
        public void AddStuff(int count)
        {
            ...
            if (this.Ready)
            {
                Log.Warning("Tried to add pickling to a pickle crock that is already fully pickled.");
                return;
            }
        ...
        public override void Draw()
        {
            ...
            if (!this.Empty)
            {
                ...
                {
                    center = drawPos,
                    size = Building_PicklingCrock.BarSize,
                    fillPercent = (float)this.stuffCount / (float)MaxCapacity,
                    filledMat = this.BarFilledMat,
                    unfilledMat = Building_PicklingCrock.BarUnfilledMat,
                    margin = 0.1f,
                    rotation = Rot4.North
                });
            }
